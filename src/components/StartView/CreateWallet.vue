  ...
  <div class="content">
    ...
    <transition name="fade">
      <div class="fields" v-if="show">
        ...
        <div class="message">
          <span :class="{ 'show': showMessage }">{{ message }}</span>
        </div>
...
export default {
  ...
  mounted() {
    ...
    window.ipc.on('WALLET_CREATED', (createdResult) => {
      console.log('wallet created:', createdResult); // TODO: Route to WalletMain view
    });
  },
  methods: {
    ...
    createWallet() {
      ...
      window.ipc.send('CHECK_DIR_EXISTS', this.selectedDirectory);
      window.ipc.once('DIR_EXISTS', (existsResult) => {
        if (this.selectedDirectory.length === 0) {
          this.showError('Wallet directory cannot be empty');
          return;
        }
